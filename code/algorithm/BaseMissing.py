...
class BaseMissing:
    ...
    def calcNormDisBtwTwoArrays(self, vals1, vals2):
        ...
        for attri in range(attrNum):
            ...
            if self.assist.isNumber(val1) and self.assist.isNumber(val2):
                numVal1 = float(val1)
                numVal2 = float(val2)
                dis = self.assist.normNumDis(numVal1, numVal2, float(self.db.values.max(axis=0)[attri]), float(self.db.values.min(axis=0)[attri]))
            else:
                dis = self.assist.normStrDis(val1, val2)
        ...
        for attri in range(attrNum):
            ...
            try:
                if self.assist.isNumber(val1) and self.assist.isNumber(val2):
                    numVal1 = float(val1)
                    numVal2 = float(val2)
                    dis = self.assist.normNumDis(numVal1, numVal2, float(self.dbVals.max(axis=0)[attri]),
                                                 float(self.dbVals.min(axis=0)[attri]))
                else:
                    dis = self.assist.normStrDis(val1, val2)
    ...
    def modify_down_stream(self, cells):
        ...
        modify_y = self.cls(modify_x, expection_n)

        return origin_y, modify_y


    def cls(self, x, n_cluster):
        y = SpectralClustering(n_clusters=n_cluster, assign_labels='kmeans', random_state=2022).fit(x).labels_
        return y
